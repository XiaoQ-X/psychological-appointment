...
后端开始从 `server/src/app.js` 拆分核心路由，但本轮不改变任何公开接口路径和响应结构：

- `server/src/routes/auth.js`：学生、咨询师、管理员登录，刷新 token，退出登录，当前用户，修改密码。
- `server/src/routes/admin-dashboard.js`：管理员 dashboard 真实聚合数据。
- `server/src/routes/appointments.js`：学生预约、咨询师预约处理、管理员预约列表。

调用方仍按本文档列出的 `/api/...` 路径访问。拆分仅影响服务端内部组织，用于降低后续权限校验、输入校验和业务测试维护成本。

新增浏览器 E2E 对以下接口链路形成端到端保护：

- 学生登录、改密、查询咨询师、查询排班、创建预约、查询预约、取消预约。
- 咨询师登录、改密、查询预约、确认、签到、完成。
- 管理员登录、dashboard、学生列表、咨询师列表、预约列表。
- 小程序 H5 登录、改密、首页加载。
