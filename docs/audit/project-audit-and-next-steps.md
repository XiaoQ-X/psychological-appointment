...
1. 身份证后六位仍作为学生/咨询师初始和重置密码。
2. 缺少首次登录强制改密、密码过期和管理员强制失效会话机制。
3. API 测试会累积业务数据，演示库和测试库未隔离。
4. 管理后台部分图表是静态概览，不是实时统计。
5. 浏览器端 JWT 存储在 `localStorage`。
6. uni-app/Vite 构建链仍有 9 个高危依赖审计项。
7. 学生 H5 和咨询师 H5 是固定手机容器，不是完整响应式 Web 产品。
8. `server/src/app.js`、学生端和管理员端入口文件过大。
...
- 增加 `mustChangePassword` 字段和修改密码接口。
- 导入/重置后强制首次登录改密。
- 增加独立 `anxin_test` 数据库，API 测试前后自动清理。
- 避免测试账号进入演示数据库。
