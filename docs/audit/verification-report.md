...
- 学生：首次登录改密、查看咨询师与排班、创建预约、查看预约、取消预约。
- 咨询师：首次登录改密、确认预约、签到、完成预约。
- 管理员：登录 dashboard，查看用户、咨询师和预约页面。
- 微信小程序 H5：登录、首次改密、首页渲染。
- 视口：`320x700`、`390x844`、`1365x900`。
- 检查项：横向溢出、主要按钮触控面积、核心页面渲染和演示闭环状态变化。

本轮失败与处理：

1. 本机 `127.0.0.1:4173` 被其他项目占用。
   - 处理：E2E 静态预览改用 `4273` 至 `4276`。
2. 学生取消预约后成功卡片仍显示旧 pending 状态。
   - 处理：创建预约缓存改为在重新拉取列表后同步服务端状态。
3. 咨询师端处理预约仍依赖静态原型详情。
   - 处理：详情页绑定真实选中预约，并调用真实状态变更接口。
4. 多端移动端触控区域不足。
   - 处理：补齐主要操作按钮不低于 44px。
5. 管理员移动端出现横向溢出。
   - 处理：修复移动端布局约束，E2E 视口检查通过。
6. 小程序 H5 的 uni-app input 包装层不能直接 `fill`。
   - 处理：E2E helper 改为定位内部 `input`/`textarea`。

未完成验证：

- 微信开发者工具和真机 mp-weixin 未验证。
- 视觉回归截图基线尚未纳入 CI。
- 依赖升级未在本轮落地，只保留风险说明和升级分支计划。
