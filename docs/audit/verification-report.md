...
| `npm run db:migrate:deploy` | 通过 | 6 个迁移，新增 `must_change_password` |
| `npm run db:seed` | 通过 | 2 个管理员、3 个校区、8 个咨询室 |
| `npm run build` | 通过 | student、counselor、admin、wechat-miniapp 全部通过 |
| `npm run test:api` 第一次 | 通过 | 35 项检查，使用并清理 `anxin_test` |
| `npm run test:api` 第二次 | 通过 | 35 项检查，重复执行仍通过 |
...
- 改密页无横向溢出、底部遮挡和小于 44px/88rpx 的主要按钮。
...
项目当前可安装、可迁移、可构建、可重复执行隔离 API 闭环，并完成学生/咨询师首次登录强制改密。尚不能视为生产就绪，主要差距是服务端会话撤销、静态管理图表、前端令牌存储、上传隔离和 uni-app 构建链风险。
